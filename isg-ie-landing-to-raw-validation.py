#This job captures the daily reconciliation for delimited files and schema validation.If failed log details and exit the process.
import sys
...
def main():
    ...
    data_delimiter = config.get('landing_zone_info', 'data_delimiter', fallback='null')
    outputvalidation_prefix = config.get('landing_zone_info', 'outputvalidation_prefix', fallback='null')
    ...
    log_manager = logging_service.LogManager(cw_loggroup=cloudwatch_log_group, cw_logstream=pre_raw_table+'_'+guid,
                                             process_key=PROCESS_KEY, client=client_logs,job=args[REGION_KEY]+'-isg-ie-landing-to-raw-validation')
    log_manager.log(message="Starting the validation job",
    ...
    try:
        ...
        if outputvalidation_prefix != 'null':
            print("File validation started")
            schema = base_table_df.schema
            input_path = 's3://' + pre_raw_bucket + '/' + pre_raw_prefix
            output_path = 's3://' + pre_raw_bucket + '/' + outputvalidation_prefix + '/' + time.strftime('%Y%m%d%H%M%S')
            df_preraw = spark.read.schema(schema).csv(path=input_path, sep=data_delimiter)
            df_preraw = df_preraw.fillna('')
            df_diff = df_preraw.subtract(base_table_df)

            if df_diff.rdd.isEmpty():
                log_manager.log(message="Data Validation completed. No differences found",
                                args={"environment": args[REGION_KEY], "feed": pre_raw_table})
            else:
                diff_count = df_diff.count()
                log_manager.log_error(message="Data Validation failed",
                                      args={"environment": args[REGION_KEY], "feed": pre_raw_table,
                                            "count_mismatch": diff_count})
                df_diff.write.json(output_path)
        else :
            log_manager.log(message="Data Validation is skipped",
                            args={"environment": args[REGION_KEY], "feed": pre_raw_table})
